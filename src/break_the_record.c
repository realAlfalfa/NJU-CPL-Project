...
void break_the_record(){
    ...
    page.texture = IMG_LoadTexture(app.renderer, ".\\res\\pictures\\Break_The_Record.jpg");
    SDL_RenderCopy(app.renderer, page.texture, NULL, NULL);

    //配置音频信息
    Mix_OpenAudio(MIX_DEFAULT_FREQUENCY, MIX_DEFAULT_FORMAT, MIX_DEFAULT_CHANNELS, 2048);
    page.bgm = Mix_LoadMUS(".\\res\\music\\break_the_record.ogg");

    printf("Play Break_The_Record Music!\n");
    ...
    SDL_Rect rect_record = {.x = 200, .y = 130};
    ...
    SDL_Event break_the_record;
    while(SDL_WaitEvent(&break_the_record)){
        switch(break_the_record.type){
            case SDL_QUIT:
            ...
            case SDL_KEYDOWN:
                ...
                Mix_FreeMusic(page.bgm);
                Mix_CloseAudio();
                if(break_the_record.key.keysym.sym != SDLK_ESCAPE)
                    do_game();
                else
                    exit(0);
                break;
